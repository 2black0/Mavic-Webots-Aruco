from controller import Robot, LED, GPS, InertialUnit, Compass, Gyro, Keyboard
...
throttlePID = PID(float(params["throttle_Kp"]), float(params["throttle_Ki"]), float(params["throttle_Kd"]), setpoint=0)
yawPID = PID(float(params["yaw_Kp"]), float(params["yaw_Ki"]), float(params["yaw_Kd"]), setpoint=float(yaw_setpoint))

targetX, targetY, target_altitude = 0.0, 0.0, 0.0
...
while robot.step(timestep) != -1:
    ...
    front_right_motor_input = (
        float(params["k_vertical_thrust"]) + vertical_input + roll_input - pitch_input - yaw_input
    )
    ...
    mavic2proHelper.motorsSpeed(
        robot, front_left_motor_input, -front_right_motor_input, -rear_left_motor_input, rear_right_motor_input
    )
